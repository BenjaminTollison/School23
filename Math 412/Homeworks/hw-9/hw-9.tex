...
\newpage 
\begin{question}
For the non--homogeneous Ordinary Differential Equation 
\begin{align*}
u''(x) - u(x) = f(x)
\end{align*}
with $u(0) = u(1) = 0$ use Duhamel's Principle to write the solution as: 
\begin{align*}
u(x) 
= \int_{0}^{1}G(x,y)f(y)dy.
\end{align*}
In particular, what is $G(x,y)$? Note that $\int_{0}^{1}(\frac{d^2}{dx^2}G(x,y)-G(x,y))f(y) dy = f(x)$. Using
this, what does $\frac{d^2}{dx^2}G(x,y)-G(x,y)$ equal? Using similar reasoning, can you determine what 
$G(0,y)$ are? What about $G(1,y)$?
\end{question}
\begin{solutionorbox}[\stretch{1}]
